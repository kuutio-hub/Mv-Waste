...
async function fetchAndParseCalendars() {
    try {
        const [wasteResponse, nameDayResponse] = await Promise.all([
        ...
        const [wasteText, nameDayText] = await Promise.all([
            wasteResponse.text(),
        ...
        allEvents = [...wasteEvents, ...nameDayEvents].sort((a,b) => a.startDate - b.startDate);
        renderCalendar();
    } catch (error) {
